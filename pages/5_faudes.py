...
st.set_page_config(
    page_title="VIGILANCE AI — Profil des Fraudes",
    page_icon="🛡️",
    layout="wide",
...
if sombre:
    ...
    rouge_alfa = "rgba(252,129,129,0.25)"
    rouge_mid  = "rgba(252,129,129,0.55)"
else:
    bg      = "#f0f2f8"
    bgcard  = "#ffffff"
    ...
    rouge_alfa = "rgba(229,62,62,0.25)"
    rouge_mid  = "rgba(229,62,62,0.55)"
...
<div style='display:flex;align-items:center;justify-content:space-between;
    ...
    <div style='display:flex;align-items:center;gap:10px'>
        <div style='width:28px;height:28px;background:{rouge}22;
                    border:1px solid {rouge}44;border-radius:6px;
...
with c1:
    ...
    <div class='kpi-carte'>
        ...
        <div class='kpi-valeur' style='color:{rouge}'>{stats['nb_fraudes']:,}</div>
        <div style='font-size:11px;color:{txt3};font-family:Space Mono'>
            Sur {stats['total']:,} transactions
        </div>
...
with c3:
    montant_moy_fraude = df_fraude["montant_transaction"].mean()
    st.markdown(f"""
    <div class='kpi-carte'>
        <div class='kpi-label'>Montant Moyen Fraude</div>
        <div class='kpi-valeur'>{montant_moy_fraude:,.0f}</div>
        <div style='font-size:11px;color:{txt3};font-family:Space Mono'>
            En francs CFA
        </div>
    </div>""", unsafe_allow_html=True)
...
with c4:
    age_moy_fraude = df_fraude["age"].mean()
    st.markdown(f"""
    <div class='kpi-carte'>
        <div class='kpi-label'>Âge Moyen Fraudeur</div>
        <div class='kpi-valeur'>{age_moy_fraude:.1f}</div>
        <div style='font-size:11px;color:{txt3};font-family:Space Mono'>
            Années
        </div>
    </div>""", unsafe_allow_html=True)
...
with col1:
    st.markdown(f"<div class='titre-section'>Fraudes par Région</div>",
                unsafe_allow_html=True)
    region_fraude = df_fraude.groupby("region").size().sort_values(ascending=True)
    fig_reg = go.Figure(go.Bar(
        x=region_fraude.values,
        y=region_fraude.index,
        orientation="h",
        marker_color=rouge_mid,
        marker_line_width=0,
        text=region_fraude.values,
    ...
    fig_reg.update_layout(
        ...
        xaxis=dict(showgrid=True, gridcolor=pgrid,
...
with col2:
    st.markdown(f"<div class='titre-section'>Fraudes par Type de Carte</div>",
                unsafe_allow_html=True)
    carte_fraude = df_fraude.groupby("type_carte").size()
    fig_carte = go.Figure(go.Pie(
        labels=carte_fraude.index,
        values=carte_fraude.values,
        hole=0.55,
        marker=dict(colors=[rouge, orange],
                    line=dict(color=pbg, width=2)),
...
with col3:
    st.markdown(f"<div class='titre-section'>Distribution des Montants (Fraude)</div>",
                unsafe_allow_html=True)
...
with col4:
    st.markdown(f"<div class='titre-section'>Score de Crédit des Fraudeurs</div>",
                unsafe_allow_html=True)
    fig_hist2 = go.Figure(go.Histogram(
        x=df_fraude["score_credit"],
        nbinsx=20,
    ...
    fig_hist2.update_layout(
        ...
        font=dict(color=pfont, family="Syne"),
        margin=dict(l=10, r=10, t=10, b=40),
        xaxis=dict(title="Score de Crédit", showgrid=False,
                   tickfont=dict(size=9, family="Space Mono")),
        yaxis=dict(showgrid=True, gridcolor=pgrid,
                   tickfont=dict(size=9, family="Space Mono")),
    )
